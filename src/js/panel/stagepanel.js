function onServer() {
    cmd.on(CommandId.fadeInWinPanel, function (param) {
        fadeInWinPanel(param.playerDataArr, param.mvp);
    });
...
function fadeInWinPanel(paramDataArr, mvp) {
    ...
    if (verifyWin(stagePanel.playerInfoArr, mvp)) {
        var isRedWin = (mvp > 3);
        ...
        for (var i = 0; i < 4; i++) {
            ...
            console.log("new player card", paramDataArr[i], playerCard.x, playerCard.y, mvpPos);
